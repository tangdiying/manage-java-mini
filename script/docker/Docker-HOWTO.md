...
依赖：docker compose v2，删除`name: oa-system`，降低`version`版本为`3.3`以下，支持`docker-compose`。
...
```shell
# 创建maven缓存volume
docker volume create --name oa-maven-repo

docker run -it --rm --name oa-maven \
    -v oa-maven-repo:/root/.m2 \
